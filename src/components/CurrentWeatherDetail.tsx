...
export default function CurrentWeatherDetail({ current }: Props) {
  return (
    <div className="flex flex-col items-center space-y-4">
      {/* was: current.condition.code */}
      <WeatherIcon code={current.weatherCode} size="xl" />
      <div className="text-center">
        <div className="text-6xl font-bold text-zinc-900 dark:text-white">
          {Math.round(current.temperature)}°F
        </div>
        {/* optional label if you have a helper */}
        {/* <p className="text-sm text-muted-foreground">{labelForCode(current.weatherCode)}</p> */}
      </div>
    </div>
  );
}